...
def _client() -> LlmChat:
    ...
    return LlmChat(api_key=key, session_id="pf-doctor-analysis", system_message=SYSTEM_PROMPT).with_model("gemini", model)


async def _stream_text(chat: LlmChat, message: UserMessage) -> str:
    parts: list[str] = []
...
async def analyze_with_llm(masked_text: str, signal_text: str, sources: list[Source]) -> AnalysisDraft:
    for attempt in range(2):
        try:
            chat = _client()
            raw = await asyncio.wait_for(_stream_text(chat, UserMessage(text=_prompt(masked_text, signal_text, sources, attempt == 1))), timeout=45)
...
async def extract_text_with_llm(content_type: str, image_bytes: bytes) -> tuple[str, str, list[str]]:
    ...
    try:
        raw = await asyncio.wait_for(_stream_text(chat, message), timeout=45)
