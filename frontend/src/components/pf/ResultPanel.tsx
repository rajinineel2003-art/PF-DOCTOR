...
import type { AnalysisResult } from "@/lib/pf";
...
export default function ResultPanel({ result, language, onPrint, onSpeak }: ResultPanelProps) {
  ...
  const category = CATEGORY_LABELS[result.category];
  return (
      ...
      <Card className="overflow-hidden border-emerald-200 bg-white shadow-sm" data-testid="result-summary-card">
        <CardHeader className="border-b border-emerald-100 bg-emerald-50/60 p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div><p className="eyebrow text-emerald-700" data-testid="result-mode-label">{result.status === "DEMO" ? "Demo Mode · predefined example" : "Live AI · backend response"}</p><CardTitle className="mt-2 text-2xl tracking-tight text-slate-900" data-testid="result-issue-title">{language === "ta" ? category.ta : result.issue_title}</CardTitle></div>
        ...
        <CardContent className="p-6"><p className="text-[15px] leading-7 text-slate-700" data-testid="result-explanation">{result.plain_language_explanation}</p><div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950" data-testid="result-source-notice">{result.source_notice || copy.independent}</div><div className="mt-4 flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={onPrint} data-testid="print-guidance-button"><FileText size={15} /> Print / save PDF</Button><Button variant="ghost" size="sm" onClick={onSpeak} data-testid="browser-speech-button"><Volume2 size={15} /> Browser speech</Button></div></CardContent>
      </Card>
      <Card data-testid="result-evidence-card"><CardHeader className="p-6 pb-3"><CardTitle className="text-base" data-testid="result-facts-heading">{copy.facts}</CardTitle></CardHeader><CardContent className="grid gap-3 p-6 pt-2 sm:grid-cols-2">{result.facts_detected.map((fact, index) => <div className="evidence-item" key={`${fact}-${index}`} data-testid={`result-fact-${index}`}>{fact}</div>)}</CardContent></Card>
      <Card data-testid="result-reasoning-card"><CardHeader className="p-6 pb-3"><CardTitle className="text-base" data-testid="result-why-heading">{copy.why}</CardTitle></CardHeader><CardContent className="p-6 pt-2"><ul className="space-y-3 text-sm leading-6 text-slate-600">{result.why_this_matches.map((item, index) => <li className="flex gap-3" key={`${item}-${index}`} data-testid={`result-reason-${index}`}><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-emerald-600" />{item}</li>)}</ul></CardContent></Card>
      <Card data-testid="result-actions-card"><CardHeader className="p-6 pb-3"><CardTitle className="text-base" data-testid="result-actions-heading">{copy.actions}</CardTitle></CardHeader><CardContent className="space-y-3 p-6 pt-2">{result.recommended_actions.map((action) => <div className="action-item" key={action.step} data-testid={`result-action-${action.step}`}><span className="action-number">{String(action.step).padStart(2, "0")}</span><div><p className="text-sm leading-6 text-slate-700">{action.action}</p><p className="mt-1 font-mono text-[10px] font-bold uppercase tracking-widest text-emerald-700">Who: {action.responsible_party}</p></div></div>)}</CardContent></Card>
      <div className="grid gap-4 sm:grid-cols-2"><Card data-testid="result-documents-card"><CardHeader className="p-6 pb-3"><CardTitle className="text-base">{copy.documents}</CardTitle></CardHeader><CardContent className="p-6 pt-2"><ul className="space-y-2 text-sm leading-6 text-slate-600">{result.documents_needed.map((item, index) => <li key={`${item}-${index}`} data-testid={`result-document-${index}`}>• {item}</li>)}</ul></CardContent></Card><Card data-testid="result-uncertainties-card"><CardHeader className="p-6 pb-3"><CardTitle className="text-base">{copy.uncertainty}</CardTitle></CardHeader><CardContent className="p-6 pt-2"><ul className="space-y-2 text-sm leading-6 text-slate-600">{result.uncertainties.map((item, index) => <li key={`${item}-${index}`} data-testid={`result-uncertainty-${index}`}>• {item}</li>)}</ul></CardContent></Card></div>
      <Card data-testid="result-sources-card"><CardHeader className="p-6 pb-3"><CardTitle className="text-base">{copy.sources}</CardTitle></CardHeader><CardContent className="space-y-3 p-6 pt-2">{result.sources.length ? result.sources.map((source) => <div className="flex items-start justify-between gap-4 rounded-lg border border-slate-200 p-3" key={source.id} data-testid={`result-source-${source.id}`}><div><p className="text-sm font-medium text-slate-800">{source.title}</p><p className="mt-1 text-xs text-slate-500">{source.organization} · {source.document_type}</p></div>{source.url ? <a className="source-link" href={source.url} target="_blank" rel="noreferrer" data-testid={`result-source-link-${source.id}`}>Open source <ExternalLink size={13} /></a> : <span className="text-xs text-slate-500">Source link unavailable</span>}</div>) : <p className="text-sm leading-6 text-slate-600" data-testid="result-source-unavailable">{copy.sourceUnavailable}</p>}</CardContent></Card>
